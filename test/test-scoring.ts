...
const approx_equal = (t, actual, expected, msg) => t.equal(truncate_float(actual), truncate_float(expected), msg);
...
test('search', function(t) {
  ...
  let msg = s => `returns one bruteforce match given an empty match sequence: ${s}`;
  ...
  matches = ([m1] = Array.from(ref1 = [m(3, 9, 1)]), ref1);
  result = scoring.most_guessable_match_sequence(password, matches, exclude_additive);
  t.equal(result.sequence.length, 2, msg("result.match.sequence.length == 2"));
  ...
  matches = ([m1] = Array.from(ref2 = [m(1, 8, 1)]), ref2);
  result = scoring.most_guessable_match_sequence(password, matches, exclude_additive);
  ...
  matches = ([m0, m1, m2] = Array.from(ref4 = [m(0, 9, 3), m(0, 3, 2), m(4, 9, 1)]), ref4);
  result = scoring.most_guessable_match_sequence(password, matches, exclude_additive);
...
test('spatial guesses', function(t) {
  ...
  for (i = 2, end = L, asc = 2 <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
    for (let j = 1, end1 = Math.min(match.turns, i-1), asc1 = 1 <= end1; asc1 ? j <= end1 : j >= end1; asc1 ? j++ : j--) {
...
test('dictionary_guesses', function(t) {
  ...
  const expected = 32 * scoring.l33t_variations(match) * scoring.uppercase_variations(match);
