...
test("search", function (t) {
  ...
  let ma: IMatch & { pattern: string } = result.sequence[0];
  t.equal(ma.pattern, "bruteforce", msg("match.pattern == 'bruteforce'"));
  t.equal(ma.token, password, msg(`match.token == ${password}`));
  t.deepEqual([ma.i, ma.j], [0, 9], msg(`[i, j] == [${ma.i}, ${ma.j}]`));
  ...
  let matches = (([m0] = Array.from((ref = [m(0, 5, 1)]))), ref);
  result = scoring.most_guessable_match_sequence(
    password,
    matches,
    exclude_additive
  );
  ...
  ma = result.sequence[1];
  t.equal(ma.pattern, "bruteforce", msg("second match is bruteforce"));
  t.deepEqual(
    [ma.i, ma.j],
    [6, 9],
  ...
  matches = (([m0] = Array.from((ref1 = [m(3, 9, 1)]))), ref1);
  result = scoring.most_guessable_match_sequence(
    password,
    matches,
    exclude_additive
  ...
  t.equal(result.sequence.length, 2, msg("result.match.sequence.length == 2"));
  ma = result.sequence[0];
  t.equal(ma.pattern, "bruteforce", msg("first match is bruteforce"));
  t.deepEqual(
    [ma.i, ma.j],
    [0, 2],
  ...
  matches = (([m0] = Array.from((ref2 = [m(1, 8, 1)]))), ref2);
  result = scoring.most_guessable_match_sequence(
    password,
    matches,
    exclude_additive
  ...
  ma = result.sequence[0];
  let m2: IMatch & { pattern: string } = result.sequence[2];
  t.equal(ma.pattern, "bruteforce", msg("first match is bruteforce"));
  ...
  matches = (([m0, m1] = Array.from((ref3 = [m(0, 9, 1), m(0, 9, 2)]))), ref3);
  result = scoring.most_guessable_match_sequence(
    password,
    matches,
    exclude_additive
  ...
  matches =
    (([m0, m1, m2] = Array.from((ref4 = [m(0, 9, 3), m(0, 3, 2), m(4, 9, 1)]))),
    ref4);
  result = scoring.most_guessable_match_sequence(
    password,
    matches,
...
test("calc_guesses", function (t) {
  ...
  let msg = "estimate_guesses returns cached guesses when available";
...
test("regex guesses", function (t) {
  ...
  t.equal(
    scoring.regex_guesses(match),
    Math.abs(scoring.REFERENCE_YEAR - 1972),
    msg
  );
...
test("date guesses", function (t) {
  ...
  t.equal(
    scoring.date_guesses(match),
    365 * Math.abs(scoring.REFERENCE_YEAR - match.year),
    msg
  );
...
test("spatial guesses", function (t) {
  let asc, i;
  let end;
  let match: ISpatialMatch = {
    token: "zxcvbn",
    graph: "qwerty",
    turns: 1,
    shifted_count: 0,
  ...
  let msg = "with no turns or shifts, guesses is starts * degree * (len-1)";
  t.equal(scoring.spatial_guesses(match), base_guesses, msg);

  match.token = "ZxCvbn";
  match.shifted_count = 2;
  let shifted_guesses = base_guesses * (nCk(6, 2) + nCk(6, 1));
  msg =
  ...
  const s = scoring.KEYBOARD_STARTING_POSITIONS;
  const d = scoring.KEYBOARD_AVERAGE_DEGREE;
  for (
    i = 2, end = L, asc = 2 <= end;
    asc ? i <= end : i >= end;
    asc ? i++ : i--
    ...
    for (
      let j = 1, end1 = Math.min(match.turns, i - 1), asc1 = 1 <= end1;
      asc1 ? j <= end1 : j >= end1;
      asc1 ? j++ : j--
    ) {
...
test("dictionary_guesses", function (t) {
  let match: IDictionaryMatch = {
    token: "aaaaa",
    rank: 32,
    pattern: "dictionary",
    dictionary_name: "dic",
  ...
  t.equal(
    scoring.dictionary_guesses(match),
  ...
  t.equal(
    scoring.dictionary_guesses(match),
    32 * scoring.l33t_variations(match),
    msg
  );
  ...
  const expected =
    32 * scoring.l33t_variations(match) * scoring.uppercase_variations(match);
...
test("uppercase variants", function (t) {
  for (let [word, variants] of [
    ...
    const msg = `guess multiplier of ${word} is ${variants}`;
    t.equal(scoring.uppercase_variations({ token: word }), variants, msg);
  }
  return t.end();
});

test("l33t variants", function (t) {
  let word;
  let msg, sub, variants;
  let match: IDictionaryMatch = {
    token: "AaA@@@",
    rank: 32,
    pattern: "dictionary",
